/* MVP סטטי — SkyTrain בלבד (Expo, Millennium, Canada) עם כל התחנות וכל ההסתעפויות.
   ...
   בשלב הבא נוכל "להזריק" זמן-אמת מהפרוקסי, בלי לשבור את המבנה הזה.
*/
...
const LINE_META = {
  EXPO: {
    ...
    headways: [
      ...
      { start: "19:00", end: "23:59", mins: 8 }
    ],
    firstTrain: "05:00", lastTrain: "00:45"
  },
  MILL: {
  ...
  CAN: {
    ...
    headways: [
      ...
      { start: "16:00", end: "18:59", mins: 4 },
      { start: "19:00", end: "23:59", mins: 6 }
    ],
    firstTrain: "05:00", lastTrain: "00:45"
  }
};

/* ===== גרף תחנות: רשימות קשתות (u,v,דקות,קו) =====
   זמני הקטעים הם קירובים טובים ל-MVP. דיוקים 1:1 יגיעו עם יבוא GTFS בשלב 2.
*/
const EDGES = [
// --- EXPO: Waterfront -> Columbia (גזע מרכזי)
...
// --- EXPO ענף סרי: Columbia -> King George
...
// --- EXPO ענף Production Way: Columbia -> Production Way–University
...
// --- MILLENNIUM: VCC–Clark -> Lafarge Lake–Douglas (דרך Lougheed)
...
// --- CANADA: Waterfront -> Bridgeport (גזע), ואז הסתעפות ל-YVR ול-Richmond
...
/* עזר ליצירת קשת דו-כיוונית */
function E(a,b,mins,line){ return {a,b,mins,line}; }

/* ===== בניית מבני נתונים מהקשתות ===== */
...
/* מוקדי החלפה טבעיים (חיתוך בין קווים) */
...
const toHHMM = mins => `${pad2(Math.floor(mins/60)%24)}:${pad2(mins%60)}`;
function headwayFor(lineId, depMins){
  const meta = LINE_META[lineId]; const t = depMins % (24*60);
  for (const w of meta.headways){ const s=toMinutes(w.start), e=toMinutes(w.end); if (s<=t && t<=e) return w.mins; }
  return meta.headways[meta.headways.length-1].mins;
}
function scheduleDeparture(lineId, earliest){
  const meta = LINE_META[lineId]; const first=toMinutes(meta.firstTrain), last=toMinutes(meta.lastTrain);
  let depart = Math.max(earliest, first); if (depart>last) return null;
  const hw = headwayFor(lineId, depart); if (depart % hw !== 0) depart += hw - (depart % hw);
  return depart<=last?depart:null;
}

/* ===== דייקסטרה בתוך קו אחד (זמן נסיעה בלבד) ===== */
function shortestOnLine(lineId, from, to){
  ...
  if (!prev.has(to) && from!==to) {
    // יתכן שאין נתיב (למשל בין ענפים לא מחוברים בקו אחר)
    if (from!==to && dist.get(to)===Infinity) return null;
  }
...
/* ===== יצירת מסלולים =====
   נשקול:
   1) קו יחיד
   2) החלפה אחת: L1->L2 בתחנה משותפת
   3) שתי החלפות: L1->L2->L3 (נדיר ב-SkyTrain אבל נשמור כאופציה)
*/
const LINES_ORDER = ["EXPO","MILL","CAN"];
const TRANSFER_MIN = 3; // דק׳
...
function planCandidates(from, to, depMins){
  ...
  for (const L1 of LINES_ORDER){
    for (const L2 of LINES_ORDER){
      if (L1===L2) continue;
      // תחנות חיתוך
      const inter = intersection(LINE_STOPS[L1], LINE_STOPS[L2]);
      for (const hub of inter){
        if (!TRANSFER_HUBS.has(hub)) continue; // לצמצם לרכזי החלפה טבעיים
  ...
  // נסה גם שיפט קטן כדי לקבל עוד חלופות
  let shift = 3;
  while (cands.length < 3 && shift <= 12){
    cands.push(...planCandidatesShifted(from, to, depMins + shift));
    shift += 3;
  }

  // סינון כפילויות ומיון: הגעה מוקדמת יותר קודמת; ואז פחות החלפות
  const uniq = new Map();
...
// עוזר לשיפט
function planCandidatesShifted(from, to, depMins){
  // מחזיר רק קו יחיד/החלפה אחת לטובת מהירות
  const out = [];
  for (const L of LINES_ORDER){
    const seg = shortestOnLine(L, from, to);
    if (seg){
...
/* ====== UI וקישור לטופס הקיים ב-index.html ====== */
...
function minutesFromDateTimeInputs(){
  ...
  return d.getHours()*60 + d.getMinutes(); // ליום הנבחר (MVP)
}
